...
//        case m: LogicalLineEvent if use_empty && m.line.isEmpty => empty_transition(config, m.line)
  ...
  case class ListState(
    ...
    private def _result: Dox = {
      ...
      // slots.head._dox_container(xs.vector)
    }
  ...
  object ListState {
    ...
    case class Slot(
      ...
      private def _dox_container(ps: Seq[Li]) = candidate.listElement match {
        case m: Ul => Ul(ps)
        case m: Ol => Ol(ps)
      }
...
//    protected def get_List_Transition(p: LogicalLine): Option[Transition] =
  ...
  case class TableState(
    ...
    private def _aligns(p: List[String]): List[Option[Table.Align]] =
      p.map(_get_align)
