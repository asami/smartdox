package org.smartdox.parser

import scalaz._
import Scalaz._
import org.scalatestplus.junit.JUnitRunner
import org.scalatest.wordspec.AnyWordSpec
import org.scalatest.matchers.should.Matchers
import org.goldenport.scalatest.ScalazMatchers
import org.goldenport.context.DateTimeContext
import org.smartdox._

/*
 * @since   Jan. 27, 2012
 *  version Feb.  5, 2014
 *  version Sep.  9, 2014
 *  version Mar. 10, 2016
 *  version Jun. 17, 2025
 *  version Aug. 16, 2025
 * @version Apr. 19, 2026
 * @author  ASAMI, Tomoharu
 */
trait UseDoxParser extends Matchers with ScalazMatchers {
  implicit protected val dateTimeContext: DateTimeContext = DateTimeContext.now()
  private def _parser = new DoxParser(false, dateTimeContext)
  private def _parser_full = new DoxParser(true, dateTimeContext)

  protected final def parse_orgmode_simple(in: String, out: String) {
    parse_orgmode(in, 
        """<!DOCTYPE html><html><head/><body>%s</body></html>""".format(out))
  }

  protected def parse_orgmode_full(in: String, out: String): Dox = {
    val result = _parser_full.parseOrgmode(in)
    result should be ('successful)
    val dox = result.get
    dox.toString() should be (out)
    dox
  }

  protected def parse_orgmode_auto_title(in: String, out: String): Dox = {
    val result = _parser.parseOrgmodeAutoTitle(in)
    result should be ('successful)
    val dox = result.get
    dox.toString() should be (out)
    dox
  }

  protected final def parse_orgmode(in: String, out: String): Dox = {
    val result = _parser.parseOrgmode(in)
    result should be ('successful)
    val dox = result.get
    dox.toString() should be (out)
    dox
  }

  protected final def parse_orgmode_z(in: String, out: String) {
    val result = _parser.parseOrgmodeZ(in)
    result should success
    result match {
      case Success(dox) => dox.toString should be (out)
      case Failure(_) => ???
    }
  }

  protected def parse_document(in: String): Document = {
    val result = _parser.parseOrgmode(in)
    result should be ('successful)
    result.get.asInstanceOf[Document]
  }

  protected def parse_document_full(in: String): Document = {
    val result = _parser_full.parseOrgmode(in)
    result should be ('successful)
    result.get.asInstanceOf[Document]
  }
}
